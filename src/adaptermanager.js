...
exports.makeBidRequests = function(adUnits, auctionStart, auctionId, cbTimeout, labels) {
  ...
  if (_s2sConfig.enabled) {
    ...
    adaptersServerSide.forEach(bidderCode => {
      ...
      const bidderRequest = {
        ...
        bidderRequestId,
        tid,
        bids: getBids({bidderCode, auctionId, bidderRequestId, 'adUnits': adUnitsS2SCopy, labels, src: CONSTANTS.S2S.SRC}),
  ...
  clientBidderCodes.forEach(bidderCode => {
    ...
    const bidderRequest = {
      ...
      auctionId,
      bidderRequestId,
      bids: getBids({bidderCode, auctionId, bidderRequestId, 'adUnits': adUnitsClientCopy, labels, src: 'client'}),
      auctionStart: auctionStart,
      timeout: cbTimeout,
      refererInfo
    };
